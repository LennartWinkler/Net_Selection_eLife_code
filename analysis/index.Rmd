...
In addition to the previouse models on reprodutive success, ran MCMC models to test for differences in the phenotypic and genetic variances in males and females for lifespan. 
...
First, we ran the MCMC testing for overall differences in phenotypic variance in  lifespan ('phenCV') between males and females ('Sex'). The model includes the species ('animal'), estimate ID ('Index') and study ID ('Study_ID') as random factors.
```{r, echo = T, results = 'hide'}
...
We examined the robustness of our models with the following model diagnostics.
The following code is an example using the MCMC for the genetic variance of reprodcutive success.
To facilitate the code for the other models, exchange 'RS_gen_MCMC_model' with the according model output.

We first ....
```{r}
# __ Model diagnostics ####

### example below: genetic variance of reprodcutive success
MCMC_model <- RS_gen_MCMC_model
library(lattice)
xyplot(log(MCMC_model$VCV))
# par(mar=c(1,1,1,1))
plot(MCMC_model, random=FALSE)
# plot(model$Sol)
# plot(model$VCV)
# plot(log(model$VCV))
```
...
```{r}
library(reshape2)
ref <- data.frame(MCMC_model$VCV)
rdf <- melt(ref, value.name="fat")
ggplot(rdf, aes(x=sqrt(fat),color=variable))+geom_density()
ggplot(ref, aes(x=sqrt(animal),y=sqrt(Study_ID)))+geom_density2d()+geom_abline(int=0,slope=1)
```

We then use Heidelberger and Welch's convergence diagnostic to ...
```{r}
# Heidelberger and Welch's convergence diagnostic 
# (https://www.rdocumentation.org/packages/coda/versions/0.19-1/topics/heidel.diag)
heidel.diag(MCMC_model$VCV)
heidel.diag(MCMC_model$Sol) 
```

Finally, we test for autocorrelations.
```{r}
# Autocorrelation Test (Lag10 should be < 0.1)
autocorr(MCMC_model$Sol) # Fixed Effects
autocorr(MCMC_model$VCV) # Random Effects
```

### Subsetting data by mating sytem

We then analyse the data separately for socially monogamous and polygamous species.
...
We then prune the phylogenetic tree to all species with data on lifespan or reproductive success for monogamous and polygamous species separately.  
...
We then run the MCMC models, first for the phenotypic variance in reproductive success in monogamous species.  
```{r, echo = T, results = 'hide'}
...
Next we run the MCMC model for the genetic variance in reproductive success in monogamous species.
```{r, echo = T, results = 'hide'}
...
Then follow the models for reproductive success in polygamous species.
First, the MCMC for the phenotypic variance in reproductive success in polygamous species.  
```{r, echo = T, results = 'hide'}
...
Next we run the MCMC model for the genetic variance in reproductive success in polygamous species.
```{r, echo = T, results = 'hide'}
...
Now follow the mating system specific MCMCs for lifespan.
We then run the MCMC models, first for the phenotypic variance in lifespan in monogamous species.  
```{r, echo = T, results = 'hide'}
...
Next we run the MCMC model for the genetic variance in lifespan in monogamous species.
```{r, echo = T, results = 'hide'}
...
Then follow the models for lifespan in polygamous species.
First, the MCMC for the phenotypic variance in lifespan in polygamous species.  
```{r, echo = T, results = 'hide'}
...
Next we run the MCMC model for the genetic variance in lifespan in polygamous species.
```{r, echo = T, results = 'hide'}
...
In this part we analysed the vertebrate species separately, as all monogamous species in this dataset are vertebrates.

We first subset our dataframe to only include vertebrates (mammalia, aves or reptilia). 
```{r, echo = T, results = 'hide'}
